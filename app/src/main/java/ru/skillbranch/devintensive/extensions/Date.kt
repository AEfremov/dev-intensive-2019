...
fun Date.humanizeDiff(date: Date = Date()): String {
    ...
    return when {
        ...
        diff <= SECOND.value * 45 -> getTenseForm("несколько секунд", isPast)
        diff <= SECOND.value * 75 -> getTenseForm("минуту", isPast)
        diff <= MINUTE.value * 45 -> getTenseForm(MINUTE.plural((diff / MINUTE.value).toInt()), isPast)
        diff <= MINUTE.value * 75 -> getTenseForm("час", isPast)
        diff <= HOUR.value * 22 -> getTenseForm(HOUR.plural((diff / HOUR.value).toInt()), isPast)
        diff <= HOUR.value * 26 -> getTenseForm("день", isPast)
        diff <= DAY.value * 360 -> getTenseForm(DAY.plural((diff / DAY.value).toInt()), isPast)
